...
assert.doesNotMatch(analysisScreenSource, /\bv\d+\.\d+\.\d+\b/)
...
const engineEntrySource = fs.readFileSync(path.join(root, '..', 'TennisHighlightAnalysis.py'), 'utf8')
...
assert.match(engineEntrySource, /import traceback/)
assert.doesNotMatch(engineEntrySource, /^from engine\.pipeline import run_analysis/m)
assert.match(engineEntrySource, /try:\s+from engine\.pipeline import run_analysis[\s\S]*run_analysis\(/)
assert.match(engineEntrySource, /except Exception as error:[\s\S]*traceback\.format_exc\(\)/)
assert.match(engineEntrySource, /json\.dumps\(\{"type": "error", "message": str\(error\), "traceback": tb\}/)
